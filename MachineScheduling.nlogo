...
to setup-patches
  ...
  set roads patches with [ pycor = 0]
  set intersections roads with [pycor = 0 and pxcor = 4]
  set machines patches with [ pycor = 0 and pxcor = 5]

  ask roads [ set pcolor white ]
  ask machines [set pcolor black]
  ask intersections  [ set pcolor red set intersection? true]
...
to setup-cars  ;; turtle procedure
  ...
  [ set heading 180 ]
  [ set heading 90 ]
  set due-date ticks + random 50
  set processing-time random 20
  set arrival-time 0
  set job-weight random 100
  set finished? false
end
...
to set-car-speed  ;; turtle procedure
  ifelse pcolor = red or pcolor = black or pxcor = 6
  [set speed 0 ]
  [set speed 0.999 ]
  if pcolor = red [set arrival-time ticks]
end
...
to schedule-orders
  ...
  if schedule-heuristic = "duedate" [schedule-by-duedate]
  if schedule-heuristic = "SPT" [schedule-by-shortest-processing-time]
  if schedule-heuristic = "job-weight" [schedule-by-job-weight]
end

to schedule-by-duedate
ask one-of machines [if not any? turtles-here
  [ask one-of intersections [
    if any? turtles-here[ask min-one-of turtles-here [due-date] [setxy 5 0]]
      ]
    ]
  ]
end
...
to schedule-by-fcfs
  ask one-of machines [if not any? turtles-here
  [ask one-of intersections [
    if any? turtles-here[ask max-one-of turtles-here [arrival-time] [setxy 5 0]]
      ]
    ]
  ]
end
...
to schedule-by-shortest-processing-time
    ask one-of machines [if not any? turtles-here
  [ask one-of intersections [
    if any? turtles-here[ask min-one-of turtles-here [processing-time] [setxy 5 0]]
      ]
    ]
  ]
end

to schedule-by-job-weight
    ask one-of machines [if not any? turtles-here
  [ask one-of intersections [
    if any? turtles-here[ask max-one-of turtles-here [job-weight] [setxy 5 0]]
      ]
    ]
  ]
end
  ...
  ask one-of machines [
    if any? turtles-here[
      ask one-of turtles-here [
        ...
        [set processing-time processing-time - 1 set wait-time 0]
        [setxy 6 0 set wait-time 0 set finished? true
...
to read-testfile
  ...
  ifelse ( file-exists? "netlogo-read.txt" )
  [
    file-open "netlogo-read.txt"
...
;TODO:
;- implement Tardiness/Earliness
;- Number of tardy/early jobs
;- production/inventory cost
; -job weight (priority factor, importance of job j relative to the other jobs)
;- Goals: financial profit or customer satisfaction?
@#$#@#$#@
GRAPHICS-WINDOW
327
10
...
PENS
"default" 1.0 0 -16777216 true "" "plot mean [due-date-deviation] of turtles"
...
377
Average Due Date Deviation
mean [due-date-deviation] of turtles
1
1
11
